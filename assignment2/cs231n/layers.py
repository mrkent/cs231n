...
def affine_backward(dout, cache):
  ...
  x_vec = x.reshape((N, -1))  # N x D=d1...dk
  dx_vec = dout.dot(w.T)      # N x D
...
def batchnorm_backward(dout, cache):
  ...
  xhat1  = x - sample_mean
  xhat = xhat1 / std
  print "kiran's xhat: %s, kent: %s" % (xhat, cache['x_hat'])
  N, _ = dout.shape
  print "Kiran's N: %s, kent: %s" % (N, x.shape[0])
  # wrt beta
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for n in range(N):
    for f in range(F):
      w_filtered = w[f, :, :, :]
      w_reshaped = w_filtered.reshape(1,-1)  # 1x(C*HH*WW)
      b_filtered = b[f]
      # print w_reshaped.shape, w_reshaped
      for h in range(H_out):
        for wid in range(W_out):
          x_filtered = x_padded[n, :, h*stride:(h*stride+HH), wid*stride:(wid*stride+WW)]
          x_reshaped = x_filtered.reshape(1,-1) # 1x(C*HH*WW)
          # print x_reshaped.shape, x_reshaped
          out[n,f,h,wid] = x_reshaped.dot(w_reshaped.T) + b_filtered
          # print "n: %s, f: %s, h: %s, w: %s, out: %s" % (n,f,h,wid,out[n,f,h,wid])
...
def conv_backward_naive(dout, cache):
  ...
  dx_padded = np.zeros_like(x_padded)  # We will trim off the dx's on the paddings later
  dx = np.zeros_like(x)
  dw = np.zeros_like(w)
  db = np.zeros_like(b)
  ...
  for i in xrange(H_prime):
    for j in xrange(W_prime):
      selected_x = x_padded[:, :, i * stride: i * stride + HH, j * stride: j * stride + WW]
      selected_shape = selected_x.shape
      for k in xrange(F):
        dw[k] += np.sum(selected_x * (dout[:, k, i, j])[:, None, None, None], axis=0)
      dx_padded[:, :, i * stride: i * stride + HH, j * stride: j * stride + WW] += \
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in xrange(H_prime):
    for j in xrange(W_prime):
      ...
      dx[:, :, i * stride: i * stride + HH, j * stride: j * stride + WW] += temp_binary * (dout[:, :, i, j])[:, :, None,
                                                                                          None]
